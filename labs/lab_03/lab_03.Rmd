...
A1: I found 92 different records- but I know if we include board of ed. and other variations there might be different answers- I think we covered a pretty wide range but there are so many different variations that there might be more that we missed.

```{r}
pg_schools <- payments |>
  filter(
    (str_detect(vendor_name, "PG COUNTY") | 
      str_detect(vendor_name, "PRINCE GEORGE'S") | 
      str_detect(vendor_name, "PRINCE GEORGES") | 
...
A2: 2024- each year has steadily grown - besides 2015 which had more than 2018, 2017, 2016 etc.
...
A3. Well the medical center is called the University of Maryland Medical Center and the St. John's medical center and it has a lot of different variations- I got 14.

```{r}
umd_capital <- payments |>
  filter(
    str_detect(vendor_name, "UNIVERSITY OF MARYLAND") &
...
A4: There was a big increase in funding in 2023, after decreasing into 2021 and rising in 2022.
...
A5: I found 17 grantees. For cleaning, you should have more standardized naming since University of Maryland can be written a lot of different ways. I think that the one that stood out to me the most was the 3 million dollar amount with the University of Maryland grantee- especially since there are so many other UMD grantees on there just spelled differently, this one just stood out to me because it was vastly larger than all the others.

```{r}
umd_zip <- md_grants_loans |>
