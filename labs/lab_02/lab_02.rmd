---
title: "lab_02"
author: "Derek Willis"
date: "2025-01-16"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Load libraries and establish settings

```{r}
# Turn off scientific notation
options(scipen=999)

# Load the tidyverse. If you have not installed the tidyverse already, remove the # from the next line and run it first.  
# install.packages('tidyverse')
library(tidyverse)
library(lubridate)
```

## Load Data

Let's keep working with the UMD courses and Maryland campaign expense data we used in the pre_lab.

```{r}
umd_courses <- read_rds("data/umd_courses.rds")
maryland_expenses <- read_csv("data/maryland_expenses.csv")
```

## Answer questions

**Q1.** How many departments have offered the courses with "Climate" in the title and at least one seat? Which department has offered the most? **A1. Atmospheric and Oceanic Science**

```{r}
climate_courses <- umd_courses |> filter(str_detect(title, "Climate") & seats >=1)|>
  group_by(department) |>
  summarise(
    count_classes = n()
  ) |>
  arrange(desc(count_classes))
climate_courses

```

**Q2.** Of the departments listed in A1, which one is the most interesting to you, and why?

**A2: University honors is interesting because it has the third most, even more than geographical sciences which seems a little crazy- but I am assuming since it covers a lot of bases across the entire school that is why.**

**Q3** Which departments offered the most courses in the Fall 2024 term? And which departments in that term have at least one syllabus posted? Answer both of those questions and then compare the top five departments - how are they different?

**A3. The departments that offered the most courses are business and management(174), school of music(158), info studies,** english, and teachhing and learning policy and leadership**. Business and Management has 106 courses with a syllabus posted and economics then jumps up to second, while school of music has 52 courses with a syllabus. Info sciences is still in the top 5 but now engineering has popped into the 5th place. But for having over 100 courses for the fall, the number is cut in half for most of them when you factor in the syllabus.**

```{r}
fall_2024_courses <- umd_courses |> filter(term == "202408") |> filter(syllabus_count >= 1) |>
group_by(department) |>
  summarise(
    count_courses = n()
  ) |>
  arrange(desc(count_courses))
fall_2024_courses


```

**Q4.** Using the Maryland campaign expenses data, change the datatype of the expenditure_date column so that it is a date and not a chr, saving the result as a new dataframe. Then create a new column for the month based on expenditure_date. Then write code that shows total amount spent each month by Wes Moore's committee (you'll need to search the data to find the exact name, and we're not looking in payee_name). Write a sentence describing the pattern of spending over time.

**A4. Moore's campaign spending went up throughout each month- and saw a lot of increases in June and October of 2021- which lines up with when he won the party primary ( I am unsure if I did this right, I kept getting errors so I had to look up why I was getting errors)**

```{r}
library(tidyverse)
library(lubridate)

maryland_expenses <- maryland_expenses |>
  mutate(
    expenditure_date = mdy(expenditure_date),
    month = floor_date(expenditure_date, "month")
  )
```

```{r}
wes_moore_monthly <- maryland_expenses |>
  filter(str_detect(committee_name, "Moore")) |>
  group_by(month) |>
  summarise(total_amount = sum(amount, na.rm = TRUE)) |>
  arrange(month)

wes_moore_monthly
```

```{r}
```

**Q5.** Using case_when(), create a column in the Maryland expenses data called `spending_location` indicating whether each record spent money in Maryland or outside Maryland. For records that are in Maryland, make the new column's value "In-state" and for all others, make it "Out of state". Then write code that shows the total amount of money spent in each category and describe the results. You can do this in one statement or break it up into two statements (saving the new column to your dataframe). **A5 There is much more instate than out of state, but there is still a substantial amount in out of state. A lot of money still goes to vendors out of state.**

```{r}
maryland_expenses_location <- maryland_expenses |>
  mutate(
    state = case_when(
      str_detect(address, " Maryland ") ~ "MD",
      str_detect(address, " California ") ~ "CA",
      str_detect(address, " Washington ") ~ "WA",
      str_detect(address, " Louisiana ") ~ "LA",
      str_detect(address, " Florida ") ~ "FL",
      str_detect(address, " North Carolina ") ~ "NC",
      str_detect(address, " Massachusetts ") ~ "MA",
      str_detect(address, " West Virginia ") ~ "WV",
      str_detect(address, " Virginia ") ~ "VA",
      .default = NA
    ),
    spending_location = case_when(
      state == "MD" ~ "In-state",
      TRUE ~ "Out of state"
    )
  )
```

```{r}
maryland_expenses_location |>
  group_by(spending_location) |>
  summarise(total_amount = sum(amount, na.rm = TRUE)) |>
  arrange(desc(total_amount))
```

**Q6.** Choose your own adventure: using either the UMD courses or campaign expenses data, ask and answer a question that is different than the previous ones. You can use one of the ones you provided in the pre_lab. **A6. QUESTION: How many journalism classes are offered each term- winter classes are very few with 2 or 4 but the main semesters clearly have more classes offered. (i know this is simple but I was struggling making a better question)**

```{r}

journalism_classes_by_term <- umd_courses |>
  filter(department == "Journalism") |>
  group_by(term) |>
  summarise(num_classes = n()) |>
  arrange(term)

journalism_classes_by_term
```
