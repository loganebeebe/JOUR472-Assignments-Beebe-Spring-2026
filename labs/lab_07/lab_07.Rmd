...
```{r}
variables_acs5 <- load_variables(2021, "acs5", cache=TRUE)

variable_finder <- variables_acs5 |>  filter(str_detect(concept, "MEDIAN AGE"))
```
...
```{r}
variable_finder <- variables_acs5 |>
  filter(str_detect(concept, "MEDIAN AGE"))

md_median_age <- get_acs( geography = "county",  variables = "B01002_001",state = "MD",  year = 2023)
```

**Q2** Which Maryland counties have a median age of at least 50? You can use the dataframe you just made to answer this, but you must write code to display only those counties with a median age of at least 50. **A2 Talbot and Worcester**
...
**A3 20902 had the highest rate...and 20707 had the lowest.**
...
zip_pop <- get_acs(
  geography = "zcta", variables = c(pop18_over="B09021_001"), 
  state = "MD", 
  year = 2019)

joined <- calls_zip |>
  mutate(zip = as.character(zip)) |>
  left_join(zip_pop, by = c("zip" = "GEOID")) |>
  rename(population = estimate)

joined <- joined |>
  mutate(rate_per_1000 = (total_calls / population) * 1000)

joined |> arrange(desc(rate_per_1000))
joined |> arrange(rate_per_1000)
```
