...
**A1: It has a URL in it that directly ties to live updates on earthquakes so it is always giving the most up to date information.**
...
**A2: 10308, each record is one earthquake that has happened the past month detected by USGS- from the columns we can gather time and the depth of the earthquake along with the magnitude and location.**
...
**A3: The deepest one is 667.197 km and its mag. is 4.30**
...
**A7: I looked at the place column and the string detected Alaska- there is 3329**
...
**A8: The two most common are earthquakes by a long shot and then quarry blasts**
...
**A9: The earthquake depth is much larger than all the others- it shocked me to see the negative depths at first but I guess that makes sense since an explosion would happen above sea level. Also crazy that the mining explosion is perfectly 5.**
...
**A10: i took it out so i could see the hour 0-23 when each earthquake happened.**
...
**A11: if the value is above 3 it will now say TRUE under serious**
...
```{r}
earthquakes |> 
  mutate(hour = hour(time), serious = mag > 3) |> 
  filter(serious == TRUE) |> 
  count(hour)

```

**A12**: Hour 2 had the least
