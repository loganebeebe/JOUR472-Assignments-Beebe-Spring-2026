...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer: A problem is a different spelling or naming of the same grantee that will make it seem like they are different when they are the same. Like adding a comma in one and no comma in another spelling of the same thing.**
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer: Now there is standardized capitalization and it removed some and standardized other punctuation or corporate suffixes.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer: It just standardized commas and suffixs so for example WAY STATION, INC. was merged into WAY STATION INC.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: there is a little bit of variation with this table but not a lot- this one changed the wording more, like region became regional- key_collision_merge kept the wording the same. n_gram merge made more liberal changes to the text.**
