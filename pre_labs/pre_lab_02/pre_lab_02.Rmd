...
**Answer The new variable keeps the original data set, so this way we can see the full data later if needed.**
...
**Answer OR gives us so much more than AND because it includes all the Theatre courses PLUS all courses from any department with at least 15 seats.**
...
**Answer: It changes from a character to a date- it allows for R to work with the values now as dates.**
...
**Answer: the new month column has dates that represent the first of each month- this happens because floor date is able to round each date down to the start of its month.**
...
**Answer the code creates two new columns called total votes and then another one called pct moore- this gives the votes moore received in each county**
...
**Answer Now, pct moore is in descending order- this makes it easier to read and see who had the higher percentage of votes.**
...
**Answer I think this can be interesting and useful because is ascending order you see moore's weakest counties and this could show some outlying data.**
...
**Answer I asked ChatGPT: "What are tidyverse string functions similar to str_to_upper"** **and it gave me str_to_lower which converts the text to lowercase and str_to_title which capitalizes the first letter of each word.**
...
**Answer I think the mutate creates a new column- state- and then it checks whether the addresses contains specific names of states. When there is a match, it assigns the states abbreviation.**
...
**Answer Yeah it makes sense because Maryland has the highest total spending since this data is about Maryland politics and committees.**
...
1.   Which departments at UMD have the most courses each year? Use filtering and grouping

2.  How does campaign spending change by month? Use mutate and floor date

3.  How many Journalism classes are offered each term?
