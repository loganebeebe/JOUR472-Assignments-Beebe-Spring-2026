...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** **No parsing errors appear when the data is reloaded using all rows.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer Vendor name is a character, date is a character and amount is a dbl.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer There are several ZIP codes with fewer than five characters which means there was probably either a leading zero or some data collection error.**
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer group by groups data by fiscal year and category, summarize counts the number of records in each group, arrange sorts it by fiscal year- some of the years are missing data have very low counts so there is some data collection error maybe.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer The smallest amounts include very low or zero dollar values, which may be errors or administrative records.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer The column names are changed to all lowercase, with spaces replaced by underscores.**
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer it cleans column names, it changes grantor to source, and mutate makes the source, grantee, and description text all uppercase.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer The output shows that duplicate rows exist in the dataset.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer The new dataframe has fewer rows than before so duplicates were removed.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer zip 5 keeps only the first five digits making all ZIP codes consistent.**
